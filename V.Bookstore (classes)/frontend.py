...
class Window:
    ...
    def __init__(self, window, filename):
        ...
        b3 = Button(window, text="Add entry", width=15, command=self.add_command)
        b3.grid(row=4, column=3)

        b4 = Button(window, text="Update selected", width=15, command=self.update_command)
        b4.grid(row=5, column=3)

        b5 = Button(window, text="Delete selected", width=15, command=self.delete_command)
    ...
    def get_selected_row(self, event):
        # global selected_tuple is unnecessary, because there is self.selected_tuple
        index = self.list1.curselection()[0]
        self.selected_tuple = self.list1.get(index)
        # print(index)
        # return selected_tuple --> there is no need to return the tuple, because it is GLOBAL
